...
The instructions explicitly invoke each command using `python` on the command
...
class TestPythonVersion(unittest.TestCase):
    ...
    `python` at the command-line (or that your environment resolves `python` to
    some python.6+ version if you have a custom setup).
